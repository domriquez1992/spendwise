...
The `auth`, `security`, and `user` packages hold authentication; the `expense` package is unchanged and simply sits behind the filter chain.
...
Base path: `/api/v1/expenses` — **all endpoints below require a `Bearer` token** (see above).
...
- Per-user data ownership and role-based authorization (`USER` / `ADMIN`)
